...
public class JobsApiMockTest extends BaseJenkinsMockTest {
    ...
    public void testTermBuildReturns404() throws Exception {
        ...
        try {
            ...
            assertTrue(status.errors().get(0).message().contains("term"));
            assertTrue(status.errors().get(0).message().contains("try stop instead"));
        } finally {
            jenkinsApi.close();
            server.shutdown();
        }
    ...
    public void testKillBuildReturns404() throws Exception {
        ...
        try {
            ...
            assertFalse(status.errors().isEmpty());
            assertEquals(status.errors().size(), 1);
            assertTrue(status.errors().get(0).message().contains("kill"));
            assertTrue(status.errors().get(0).message().contains("try stop instead"));
        } finally {
            jenkinsApi.close();
            server.shutdown();
